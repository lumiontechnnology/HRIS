  ...
  ({ mediaItems, className, ...props }, ref) => {
    ...
    return (
      <>
        ...
        <section ref={ref} className={`grid grid-cols-3 gap-4 border-0 p-0 bg-transparent ${className || ''}`} {...props}>
          {itemsToRender.map((item, index) => (
            <figure key={index} style={{ clipPath: `url(#${item.clipId})` }}>
              {item.type === 'image' ? (
                <img src={item.src} alt={item.alt} className="transition-all duration-300 aspect-[4/5] min-h-full align-bottom object-cover hover:scale-105 w-full" />
              ) : (
                <video autoPlay muted loop className="transition-all duration-300 aspect-[4/5] min-h-full align-bottom object-cover hover:scale-105 w-full">
